...
public class ActionEngine extends TestEngine {
	...
	public static boolean click(By locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				logger.info("Unable to click on "+locator);
	...
	{	
		...
		try {
			...
			if (flag!=true) {
				...
				logger.info("Able to click on "+title);
	...
	public static boolean scrollToText(final String text)
		...
		try {
			if(browser.toLowerCase().contains("iphone")){
				//Iosdriver.swipe(arg0, arg1, arg2, arg3, arg4);
	...
	public static boolean type(By locator, String testdata, String locatorName)
		...
		try { 
			...
			Iosdriver.findElement(locator).sendKeys(testdata);
...
//			}
			...
			if (!flag) {
				logger.info("Typing is not performed on "+locatorName);
				Reporter.failureReport("Type ",
						"Data typing action is not perform on " + locatorName
								+ " with data is " + testdata);
			} else if (b && flag) {
				logger.info("Typing is performed on "+locatorName);
	 ...
	 * 
	 */
	public static boolean type_mobile(String locator, String testdata, String locatorName)
			throws Throwable {
	//	explicityWait(locator, locatorName);
	//	Thread.sleep(2000);
		boolean flag = false;
		try { 
			...
			MobileElement we1 =(MobileElement) Iosdriver.findElementByAccessibilityId(locator);
			we1.sendKeys(testdata);
			flag = true;
			if(browser.toLowerCase().contains("android")){
				try{
...
//			}
	...
	public static boolean scrollKeysForAndroid(String[] list, String locatorname,String Passenger) throws Throwable {
        ...
        try { 
        	if((locatorname.equalsIgnoreCase("Date Of Birth")))
        	{
        		...
        		{
        			driver.findElement(By.xpath("//*[@resource-id='com.flynas.android.app:id/customDatePicker']//android.widget.LinearLayout[@index='0']/android.widget.NumberPicker[@index='2']/android.widget.EditText[contains(@resource-id,'android:id/numberpicker_input')]")).clear();
                	driver.findElement(By.xpath("//*[@resource-id='com.flynas.android.app:id/customDatePicker']//android.widget.LinearLayout[@index='0']/android.widget.NumberPicker[@index='2']/android.widget.EditText[contains(@resource-id,'android:id/numberpicker_input')]")).sendKeys(list[2]);
        		}
        		else	
        		...
        		else
        		{
        			...
        			driver.findElementByXPath("//android.widget.NumberPicker[@index='2']//android.widget.Button[@index=0]").sendKeys(list[i]);
        			//driver.findElementByXPath("//android.widget.NumberPicker[@index='2']//[@resource-id='android:id/numberpicker_input']").sendKeys(list[i]);
               	}
        	}
        	}
        	else
        		if((locatorname.equalsIgnoreCase("Credit Card Expiry Date")))
        		{
        			driver.findElementByXPath("//android.widget.NumberPicker[@index=1]//android.widget.Button[@index=2]").sendKeys(list[1]);
        		}
        	else
        	{
        		for (int i = list.length-1 ; i >=0 ; i--) {
                	driver.findElementByXPath("//android.widget.NumberPicker[@index='"+i+"']//android.widget.Button[@index=1]").sendKeys(list[i]);
                	break;
                       	}
        	}
	...
	public static void swipeVertical(float value)
	{
